...
function purgeSession() {
  let prekey = [];
  let directorio = readdirSync("./RageSession");
  let filesFolderPreKeys = directorio.filter(file => file.startsWith('pre-key-'));
  prekey = [...prekey, ...filesFolderPreKeys];
  filesFolderPreKeys.forEach(files => unlinkSync(`./RageSession/${files}`));
}
...
function purgeOldFiles() {
  const directories = ['./RageSession/', './jadibts/'];
...
async function connectionUpdate(update) {
  ...
  if (reason == 405) {
    await fs.unlinkSync("./RageSession/" + "creds.json");
